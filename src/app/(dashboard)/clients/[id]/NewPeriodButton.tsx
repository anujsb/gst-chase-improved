// src/app/(dashboard)/clients/[id]/NewPeriodButton.tsx
"use client";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm"
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-xl w-80 p-6"
            ...
            <div className="space-y-3 mb-6">
              <div>
                <label htmlFor="period-month" className="block text-xs font-medium text-zinc-700 mb-1.5">Month</label>
                <select
                  id="period-month"
                  title="Select month"
                  value={month}
                  onChange={(e) => setMonth(+e.target.value)}
                  className="w-full px-3 py-2 text-sm bg-white border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-zinc-900/10">
                  {MONTHS.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
                </select>
              </div>
              <div>
                <label htmlFor="period-year" className="block text-xs font-medium text-zinc-700 mb-1.5">Year</label>
                <select
                  id="period-year"
                  title="Select year"
                  value={year}
