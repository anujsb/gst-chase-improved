...
export default async function ReconcilePage({ params }: { params: { id: string; periodId: string } }) {
  ...
  const [purchases, gstr2b] = await Promise.all([
    db.query.purchaseInvoices.findMany({ where: eq(purchaseInvoices.filingPeriodId, params.periodId) }),
    db.query.gstr2bInvoices.findMany({ where: eq(gstr2bInvoices.filingPeriodId, params.periodId) }),
  ]);
  ...
  return (
    <div>
      ...
      <ReconcileClient
        ...
        savedCounts={{
          matched:  period.matchedCount  ?? 0,
          partial:  period.partialCount  ?? 0,
          mismatch: period.mismatchCount ?? 0,
        }}
      />
    </div>
  );
