...
import { processInvoiceFiles, deleteInvoice, getInvoicesForPeriod, type ProcessResult } from "@/lib/actions/invoices";
...
export default function InvoicePageClient({ filingPeriodId, clientId, periodLabel, initialInvoices }: Props) {
  ...
  function addFiles(files: FileList | File[]) {
    ...
    setQueue((q) => [...q, ...valid.map((f) => ({ id: crypto.randomUUID(), file: f, status: "queued" as const }))]);
  }
  ...
  async function processQueue() {
    ...
    for (const q of queued) {
      ...
      try {
        ...
        // Immediately refresh invoice list from DB
        const updated = await getInvoicesForPeriod(filingPeriodId);
        setInvoices(updated);
      } catch (err) {
        setQueue((prev) => prev.map((x) => x.id === q.id ? { ...x, status: "error", result: { fileId: "", fileName: q.file.name, status: "error", error: String(err) } } : x));
      }
    }
    setProcessing(false);
  }
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      >
        ...
        {queuedCount > 0 && (
          <div className="absolute top-3 right-3 bg-zinc-900 text-white text-xs px-2 py-1 rounded-full">
            {queuedCount} ready
          </div>
        )}
      </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-4 text-xs text-zinc-500">
              <span>Tax: <span className="font-semibold text-zinc-900">₹{totalTax.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></span>
              <span>Total: <span className="font-semibold text-zinc-900">₹{totalAmount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></span>
            </div>
          </div>
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-zinc-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex px-1.5 py-0.5 rounded text-[10px] font-medium ${conf >= 80 ? "bg-emerald-50 text-emerald-700" : conf >= 50 ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-600"}`}>
                          {conf}%
                        </span>
                      </td>
                      <td className="px-4 py-3">
          ...
          <div className="mt-4 flex justify-end">
            <a
              href={`/clients/${clientId}/periods/${filingPeriodId}/gstr2b`}
