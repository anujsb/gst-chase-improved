...
import { getClientById, GST_STATES } from "@/lib/actions/clients";
...
export default async function ClientDetailPage({ params }: { params: { id: string } }) {
  ...
  const stateName = client.stateCode ? GST_STATES[client.stateCode] : null;
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-xl bg-zinc-900 flex items-center justify-center shrink-0">
              <span className="text-lg font-semibold text-white">{client.name.charAt(0).toUpperCase()}</span>
            </div>
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                {stateName && <span className="text-xs text-zinc-500">{stateName}</span>}
      ...
      <div>
        ...
        {client.filingPeriods.length === 0 ? (
          <div className="bg-white border border-dashed border-zinc-300 rounded-xl p-10 text-center">
            ...
            <p className="text-xs text-zinc-400 mt-1">Create a period to start uploading invoices and GSTR-2B.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {client.filingPeriods.map((period) => (
              <Link
                key={period.id}
                href={`/clients/${params.id}/periods/${period.id}`}
              ...
              >
                <div className="w-20 shrink-0">
                  <p className="text-sm font-semibold text-zinc-900">{MONTHS[period.month - 1]} {period.year}</p>
                  <p className="text-xs text-zinc-400">{FULL_MONTHS[period.month - 1]}</p>
                </div>

                <span className={`text-xs w-24 ${
                  period.status === "filed" ? "text-emerald-600" :
                  period.status === "in_progress" ? "text-amber-600" :
                  period.status === "reconciled" ? "text-blue-600" : "text-zinc-400"
                }`}>
                  {period.status.replace("_", " ")}
                </span>

                <div className="flex-1 flex items-center gap-4 text-xs">
                  <span><span className="font-semibold text-zinc-700 tabular-nums">{period.totalInvoicesInBooks ?? 0}</span> <span className="text-zinc-400">invoices</span></span>
                  <span><span className="font-semibold text-zinc-700 tabular-nums">{period.totalInvoicesIn2B ?? 0}</span> <span className="text-zinc-400">in 2B</span></span>
                  {(period.matchedCount ?? 0) > 0 && <span><span className="font-semibold text-emerald-600 tabular-nums">{period.matchedCount}</span> <span className="text-zinc-400">matched</span></span>}
                  {(period.mismatchCount ?? 0) > 0 && <span><span className="font-semibold text-red-500 tabular-nums">{period.mismatchCount}</span> <span className="text-zinc-400">issues</span></span>}
                </div>
