...
export async function processInvoiceFiles(
  ...
  for (const file of files) {
    const [fileRecord] = await db
      ...
      .values({ filingPeriodId, fileName: file.name, fileType: file.type, fileSize: file.size, purpose: "invoice" })
      .returning();
    ...
    try {
      ...
      await db.update(uploadedFiles)
        .set({ processedAt: new Date(), extractedCount: 1 })
...
export async function updateInvoice(invoiceId: string, data: {
  invoiceNumber?: string; invoiceDate?: string; supplierGstin?: string;
  supplierName?: string; taxableAmount?: string; igst?: string; cgst?: string;
  sgst?: string; totalAmount?: string;
}) {
  await db.update(purchaseInvoices).set({ ...data, updatedAt: new Date() }).where(eq(purchaseInvoices.id, invoiceId));
}